...
static inline size_t roundup(size_t n) {
    return (n + (ALIGN - 1)) & ~(size_t)(ALIGN - 1);
}
static inline word_t pack(size_t sz, int alloc) {
    return (word_t)sz | (word_t)(alloc & 1);
}
static inline size_t blk_size(word_t *hdr) {
    return (size_t)(*hdr & ~(word_t)1);
}
static inline int    blk_alloc(word_t *hdr) {
    return (int)(*hdr & 1);
}
static inline word_t *hdr_of(void *payload) {
    return (word_t *)((uint8_t *)payload - WSIZE);
}
static inline void *payload_of(word_t *hdr) {
    return (uint8_t *)hdr + WSIZE;
}
static inline word_t *ftr_of(word_t *hdr) {
    return (word_t *)((uint8_t *)hdr + blk_size(hdr) - WSIZE);
}
static inline word_t *next_hdr(word_t *hdr) {
    return (word_t *)((uint8_t *)hdr + blk_size(hdr));
}
static inline word_t *prev_ftr(word_t *hdr) {
    return (word_t *)((uint8_t *)hdr - WSIZE);
}
...
void fatlfs_arena_init(void *base, size_t size) {
    ...
    g_used = g_peak = 0;
    if (stop <= start + 4 * WSIZE) {
        g_base = g_end = (uint8_t *)start;
        return;
    }
...
void *fatlfs_arena_realloc(void *p, size_t n) {
    ...
    if (n <= cur) {
        return p;                                  // fits (don't bother shrinking)
    }
