...
if "esp32" in sys.platform and not (
    "ESP32S2" in sys.implementation._machine or "ESP32C3" in sys.implementation._machine
):
    # The ESP32 needs separate UART instances for the test
    recv_uart_id = 1
    recv_tx_pin = 14
    recv_rx_pin = 5
    send_uart_id = 2
...
for bits_per_s in (2400, 9600, 57600):
    if recv_tx_pin is None:
        recv_uart = UART(recv_uart_id, bits_per_s)
        if recv_uart_id != send_uart_id:
            send_uart = UART(send_uart_id, bits_per_s)
        else:
            send_uart = recv_uart
