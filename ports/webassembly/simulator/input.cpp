...
// GPIO numbers for the user buttons (see tufty2350/board/pins.csv). The
// simulated states are sampled from the machine module.
#define GPIO_BUTTON_A    7
#define GPIO_BUTTON_B    9
#define GPIO_BUTTON_C    10
#define GPIO_BUTTON_UP   11
#define GPIO_BUTTON_DOWN 6
#define GPIO_BUTTON_HOME 22
...
// Implemented in machine.cpp: sample a simulated GPIO (active-low for buttons).
extern int simulator_gpio_get(int gpio);
...
#elif __EMSCRIPTEN__
    // Sample the simulated button GPIOs from the machine module. Buttons are
    // active-low, so a GPIO reading of 0 means pressed.
    buttons |= simulator_gpio_get(GPIO_BUTTON_A)    ? 0 : BUTTON_A;
    buttons |= simulator_gpio_get(GPIO_BUTTON_B)    ? 0 : BUTTON_B;
    buttons |= simulator_gpio_get(GPIO_BUTTON_C)    ? 0 : BUTTON_C;
    buttons |= simulator_gpio_get(GPIO_BUTTON_UP)   ? 0 : BUTTON_UP;
    buttons |= simulator_gpio_get(GPIO_BUTTON_DOWN) ? 0 : BUTTON_DOWN;
    buttons |= simulator_gpio_get(GPIO_BUTTON_HOME) ? 0 : BUTTON_HOME;
#endif
