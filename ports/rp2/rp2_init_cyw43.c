...
mp_obj_t rp2_network_cyw43_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
    enum { ARG_interface, CYW43_INIT_ARG_ENUM };
...
void machine_pin_ext_obj_init(mp_arg_val_t *args) {
    assert(args);
    enum { ARG_mode, ARG_pull, ARG_value, ARG_alt, CYW43_INIT_ARG_ENUM }; // extends the enum used for machine_pin_obj_init_helper
    uint pins[] = {
        CYW43_PIN_ARRAY
    };
