...
typedef struct { int32_t node;
                 uint32_t idx;
} owner_t;                                               // node into fs->committed
...
static uint16_t next_utf16(const char **pp) {
    ...
    if (*p < 0x80) {
        ...
        u = ((uint32_t)(p[0] & 0x1F) << 6) | (p[1] & 0x3F);
        p += 2;
    } else if ((*p & 0xF0) == 0xE0) {
        u = ((uint32_t)(p[0] & 0x0F) << 12) | ((uint32_t)(p[1] & 0x3F) << 6) | (p[2] & 0x3F);
        p += 3;
    } else {
        u = '_';
        p += 4;
...
static void read_cluster_base(fatlfs_t *fs, uint32_t c, uint8_t *buf) {
    ...
    if (c < fs->cc2 && fs->owner[c].node == -2) {
        ...
        if (!fs->spill_rd_open || fs->spill_rd_head != fs->spillhead[c]) {
            if (fs->spill_rd_open) {
                lfs2_file_close(fs->lfs, &fs->spill_rd_file);
                fs->spill_rd_open = 0;
            }
            char sp[FATLFS_PATH_MAX];
            spill_name(sp, fs->spillhead[c]);
            if (lfs2_file_opencfg(fs->lfs, &fs->spill_rd_file, sp, LFS2_O_RDONLY, &fs->fc_spill_rd) >= 0) {
                fs->spill_rd_open = 1;
                fs->spill_rd_head = fs->spillhead[c];
            } else {
...
static void read_cluster(fatlfs_t *fs, uint32_t c, uint8_t *buf) {
    if (c < fs->cc2 && fs->staging[c]) {
        memcpy(buf, fs->staging[c], fs->cluster_size);
        return;
    }
...
static void serve_fat_sector(fatlfs_t *fs, uint32_t fatsec, uint8_t *buf) {
    ...
    for (uint32_t i = 0; i < FAT_ENTRIES_PER_SEC; i++) {
        ...
        uint16_t v;
        if (e == 0) {
            v = 0xFF00 | FAT_MEDIA_FIXED;                // 0xFFF8
        } else if (e == 1) {
            v = FAT16_EOC;                               // EOC / clean bits
        } else if (e >= fs->cc2) {
            v = 0;
        } else {
            ...
            if (iv == 0) {
                ...
                v = FAT16_BAD;
            } else if (FAT_IS_EOC(iv) || iv >= fs->cc2) {
                v = FAT16_EOC;
            } else {
                v = (uint16_t)iv;
            }
...
static void apply_fat_sector(fatlfs_t *fs, uint32_t fatsec, const uint8_t *buf) {
    ...
    for (uint32_t i = 0; i < FAT_ENTRIES_PER_SEC; i++) {
        ...
        if (v == 0) {
            ...
            iv = v;    // out-of-range cluster numbers end chains in the walks
        }
        fs->fat[e] = iv;
    }
}
...
static void spill_name(char *out, uint32_t head) {
    snprintf(out, FATLFS_PATH_MAX, SPILL_PREFIX "%u", (unsigned)head);
}

static void spill_close(fatlfs_t *fs) {
    if (fs->spill_open) {
        lfs2_file_close(fs->lfs, &fs->spill_file);
        fs->spill_open = 0;
    }
}
...
static int spill_head_live(fatlfs_t *fs, uint32_t head) {
    for (uint32_t c = 2; c < fs->cc2; c++) {
        if (fs->owner[c].node == -2 && fs->spillhead[c] == head) {
            return 1;
        }
    }
    return 0;
...
static void spill_pressure(fatlfs_t *fs, uint32_t budget) {
    ...
    for (uint32_t x = 2; x < fs->cc2; x++) {
        ...
        if (nx >= 2 && nx < fs->cc2 && !FAT_IS_EOC(nx)) {
            prev[nx] = x;
        }
    }
    uint32_t moved = 0;
    for (uint32_t c = 2; c < fs->cc2 && fs->staged_bytes > fs->staging_max; c++) {
        ...
        if (fs->dir_bytes[c]) {
            continue;                                             // never spill known dirs
        }
        uint32_t idx, head = chain_head_of(fs, c, &idx, prev);
        if (idx == 0) {
            continue;                                             // keep chain heads in RAM
        }
        if (!fs->spill_open || fs->spill_head != head) {
            spill_close(fs);
            char sp[FATLFS_PATH_MAX];
            spill_name(sp, head);
            if (lfs2_file_opencfg(fs->lfs, &fs->spill_file, sp, LFS2_O_WRONLY | LFS2_O_CREAT, &fs->fc_spill) < 0) {
                continue;
            }
            fs->spill_open = 1;
            fs->spill_head = head;
        ...
        if (!wrote) {
            break;           // lfs full / IO error: the staged copy is the ONLY copy -
        }
...
static void read_sector(fatlfs_t *fs, uint64_t lba, uint8_t *buf) {
    if (lba < fs->reserved_sectors) {
        if (lba == 0) {
            memcpy(buf, fs->boot, FAT_SECTOR_SIZE);
        } else {
...
static int write_sector(fatlfs_t *fs, uint64_t lba, const uint8_t *buf) {
    if (lba < fs->reserved_sectors) {
        return FATLFS_OK;                                 // boot sector: accept, ignore
    }
    if (lba < fs->root_start) {
        uint32_t rel = (uint32_t)(lba - fs->reserved_sectors);
    ...
    if (!st) {
        return FATLFS_ERR_NOMEM;
    }
    memcpy(st + secoff, buf, FAT_SECTOR_SIZE);
    if (!fs->no_inline_spill && fs->staged_bytes > fs->staging_max) {
        spill_pressure(fs, 0);
    }
    return FATLFS_OK;
}
...
static void decode_short(const uint8_t *e, char *out) {
    char base[9], ext[4];
    int bn = 0, en = 0;
    for (int i = 0; i < 8; i++) {if (e[i] != ' ') {
                                     base[bn++] = e[i];
                                 } else {
                                     break;
                                 }}
    base[bn] = 0;
    for (int i = 8; i < 11; i++) {if (e[i] != ' ') {
                                      ext[en++] = e[i];
                                  } else {
                                      break;
                                  }}
    ext[en] = 0;
    if (bn && (uint8_t)base[0] == DIRENT_KANJI_E5) {
        base[0] = (char)0xE5;
    }
    if (e[DIR_NTRes] & 0x08) {
        for (int i = 0; i < bn; i++) { base[i] = (char)tolower((unsigned char)base[i]);
        }
    }
    if (e[DIR_NTRes] & 0x10) {
        for (int i = 0; i < en; i++) { ext[i] = (char)tolower((unsigned char)ext[i]);
        }
    }
    if (en) {
        snprintf(out, FATLFS_LFN_MAX + 1, "%s.%s", base, ext);
    } else {
        snprintf(out, FATLFS_LFN_MAX + 1, "%s", base);
    }
}
...
typedef struct { fatlfs_node_t *v;
                 int n, cap;
} nodevec;
...
static int scan_dir_entries(fatlfs_t *fs, dirscan_t *sc, const uint8_t *buf, uint32_t nents) {
    ...
    for (uint32_t i = 0; i < nents; i++) {
        ...
        if ((attr & ATTR_LONG_MASK) == ATTR_LONG_NAME) {
            ...
            if (last) {
                sc->have_lfn = 1;
                sc->lfn_csum = e[LDIR_Chksum];
                sc->lfn_len = seq * LFN_CHARS_PER_ENTRY;
            }
            if (!sc->have_lfn) {
                continue;
            }
        ...
        memcpy(shortname, e, 11);
        if (sc->have_lfn && sc->lfn_csum == fat_lfn_checksum(shortname)) {
            char *p = s->name;
            for (int k = 0; k < sc->lfn_len; k++) { if (s->lfn[k] == 0) {
                                                        break;
                                                    }
                                                    put_utf8(&p, s->lfn[k]);
            }
        ...
        if (s->name[0] == '.' && (s->name[1] == 0 || (s->name[1] == '.' && s->name[2] == 0))) {
            continue;
        }
...
static int parse_dir(fatlfs_t *fs, uint32_t start, const char *prefix,
    ...
    if (start == 0) {
        ...
        while (c >= 2 && c < fs->cc2 && guard--) {
            ...
            if (nxt < 2 || nxt >= fs->cc2 || FAT_IS_EOC(nxt)) {
                break;
            }
            c = nxt;
        }
...
static int find_prev(fatlfs_t *fs, const fatlfs_node_t *nn) {
    for (int i = 0; i < fs->committed_n; i++) {
        ...
        if (p->is_dir != nn->is_dir) {
            continue;
        }
        if (nn->first_cluster != 0 && p->first_cluster == nn->first_cluster) {
            return i;
        }
    }
    if (nn->first_cluster == 0 && !nn->is_dir) {
        for (int i = 0; i < fs->committed_n; i++) {
            fatlfs_node_t *p = &fs->committed[i];
            if (!p->is_dir && p->first_cluster == 0 && strcmp(p->path, nn->path) == 0) {
                return i;
            }
        }
    }
...
static int flush_file_data(fatlfs_t *fs, const fatlfs_node_t *f) {
    ...
    if (f->first_cluster) {
        spill_rd_close(fs);
        char sp[FATLFS_PATH_MAX];
        spill_name(sp, f->first_cluster);
        lfs2_remove(fs->lfs, sp);
    }
    return (r < 0) ? r : rc;
}
...
static int name_is_trash(const char *name) {
    if (name[0] == '.' && name[1] == '_') {
        return 1;                                     // ._* AppleDouble sidecars
    }
    ...
    for (int i = 0; trash[i]; i++) {if (strcmp(name, trash[i]) == 0) {
                                        return 1;
                                    }
    }
    return 0;
...
static int reconcile(fatlfs_t *fs) {
    ...
    for (int oi = 0; oi < nv.n; oi++) {
        ...
        if (r == LFS2_ERR_NOENT) {
            ...
            struct lfs2_info st;
            if (lfs2_stat(fs->lfs, tmp, &st) >= 0) {
                r = 0;
            } else if (lfs2_stat(fs->lfs, nv.v[i].path, &st) >= 0) {
                r = 0;
                tmp[0] = 0;
            }
        }
        ...
        for (int oi = 0; oi < fs->committed_n; oi++) {
            ...
            for (int j = 0; j < nv.n; j++) {
                if (match[j] < 0 && nv.v[j].is_dir == fs->committed[i].is_dir &&
                    strcmp(nv.v[j].path, fs->committed[i].path) == 0) {
                    reused = 1;
                    break;
                }
            }
    ...
    // left alone rather than truncated into deleting the wrong path.
    {
        ...
        if (lfs2_dir_open(fs->lfs, &d, "/") >= 0) {
            ...
            while (lfs2_dir_read(fs->lfs, &d, &inf) > 0 && nvic < 16) {
                if (strncmp(inf.name, SPILL_PREFIX, strlen(SPILL_PREFIX)) != 0) {
                    continue;
                }
                size_t len = strlen(inf.name);
                if (len >= sizeof victims[0]) {
                    continue;
                }
                uint32_t head = (uint32_t)strtoul(inf.name + strlen(SPILL_PREFIX), NULL, 10);
                if (spill_head_live(fs, head)) {
                    continue;
                }
    ...
    for (uint32_t c = 2; c < fs->cc2; c++) {
        if (dircls[c]) {
            if (!fs->dir_bytes[c]) {
                fs->dir_bytes[c] = malloc(fs->cluster_size);
            }
            if (fs->dir_bytes[c]) {
                read_cluster(fs, c, fs->dir_bytes[c]);
            }
...
done2:
    if (tmpname) {
        for (int i = 0; i < nv.n; i++) { free(tmpname[i]);
        }
    }
    free(match);
    free(prev_used);
...
static int build_snapshot(fatlfs_t *fs) {
    ...
    memcpy(fs->committed, nv.v, nv.n * sizeof(fatlfs_node_t));
    fs->committed_n = nv.n;
    for (uint32_t c = 0; c < fs->cc2; c++) { fs->owner[c].node = -1;
                                             fs->owner[c].idx = 0;
                                             fs->spillhead[c] = 0;
    }
    for (int i = 0; i < fs->committed_n; i++) {
...
typedef struct { fatlfs_t *fs;
                 uint32_t first, cur, off;
                 int root;
} dirwriter;
...
static int dw_start(dirwriter *dw, fatlfs_t *fs, uint32_t fixed_first) {
    ...
    if (!fs->dir_bytes[dw->cur]) {
        fs->dir_bytes[dw->cur] = calloc(1, fs->cluster_size);
    } else {
        memset(fs->dir_bytes[dw->cur], 0, fs->cluster_size);
    }
...
static int dw_put(dirwriter *dw, const uint8_t *ent) {
    ...
    if (dw->root) {
        ...
        if (dw->off + FAT_DIRENT_SIZE > fs->root_entries * FAT_DIRENT_SIZE) {
            return FATLFS_ERR_NOMEM;
        }
    ...
    if (dw->off + FAT_DIRENT_SIZE > fs->cluster_size) {
        ...
        fs->fat[nc] = FAT_EOC;
        if (!fs->dir_bytes[nc]) {
            fs->dir_bytes[nc] = calloc(1, fs->cluster_size);
        } else {
            memset(fs->dir_bytes[nc], 0, fs->cluster_size);
        }
        if (!fs->dir_bytes[nc]) {
            return FATLFS_ERR_NOMEM;
        }
...
static void gen_short(const char *name, uint8_t out[11], char used[][11], int *nused) {
    ...
    for (int n = 1; n < 1000000; n++) {
        char num[8];
        int nl = snprintf(num, sizeof num, "~%d", n);
        int keep = 8 - nl;
        if (keep > bn) {
            keep = bn;
        }
        ...
        memcpy(out + 8, ext, en);
        int clash = 0;
        for (int i = 0; i < *nused; i++) {if (memcmp(used[i], out, 11) == 0) {
                                              clash = 1;
                                              break;
                                          }
        }
        if (!clash) {
            ...
            // resolve by LFN.
            if (*nused < SHORT_TABLE_MAX) {
                memcpy(used[*nused], out, 11);
                (*nused)++;
            }
            return;
        }
...
static int emit_entry(dirwriter *dw, const char *name, int is_dir,
    ...
    uint16_t u16[FATLFS_LFN_MAX + 16];
    int ulen = 0;
    const char *p = name;
    while (ulen < FATLFS_LFN_MAX) {
        uint16_t u = next_utf16(&p);
        u16[ulen++] = u;
        if (u == 0) {
            break;
        }
    }
    if (ulen == 0 || u16[ulen - 1] != 0) {
        u16[ulen++] = 0;
    }
    int nlfn = (ulen + 12) / 13;
...
static int encode_dir(fatlfs_t *fs, const char *lpath, uint32_t this_cluster, uint32_t parent_cluster) {
    ...
    struct { char name[FATLFS_LFN_MAX + 1];
             uint32_t size;
             uint8_t is_dir;
    } *kids = NULL;
    ...
    if (this_cluster != 0) {   // the fixed root region carries no "."/".."
        ...
        fat_dirent_set_cluster(e, dw.first);
        if ((r = dw_put(&dw, e))) {
            goto out;
        }
...

    {
        ...
        while (lfs2_dir_read(fs->lfs, &d, &s->info) > 0) {
            ...
            if (strncmp(nm, SPILL_PREFIX, strlen(SPILL_PREFIX)) == 0) {
                continue;
            }
            if (strncmp(nm, MV_PREFIX, strlen(MV_PREFIX)) == 0) {
                continue;
            }
            if (nk == capk) {
            ...
            kids[nk].size = s->info.size;
            kids[nk].is_dir = (s->info.type == LFS2_TYPE_DIR);
            nk++;
        }
        lfs2_dir_close(fs->lfs, &d);
        r = 0;
    ...
    for (int i = 0; i < nk; i++) {
        if (lpath[0]) {
            snprintf(s->childpath, sizeof s->childpath, "%s/%s", lpath, kids[i].name);
        } else {
            snprintf(s->childpath, sizeof s->childpath, "%s", kids[i].name);
        }
        if (kids[i].is_dir) {
            ...
            if ((r = emit_entry(&dw, kids[i].name, 1, sub, 0, s->used, &nused))) {
                goto out;
            }
            if ((r = encode_dir(fs, s->childpath, sub, dw.first))) {
                goto out;
            }
        } else {
            uint32_t first = 0;
            ...
            if ((r = emit_entry(&dw, kids[i].name, 0, first, kids[i].size, s->used, &nused))) {
                goto out;
            }
        }
    }
    ...
    for (int pass = 0; pass < 64; pass++) {
        ...
        for (int i = 0; i < nvic; i++) {
            ...
            for (n = 0; n < 1000; n++) {
                ...
                if (lfs2_stat(fs->lfs, dst, &st) < 0) {
                    break;
                }
            }
            if (n >= 1000 || lfs2_rename(fs->lfs, victims[i], dst) < 0) {
                return;
            }
        }
    }
...
int fatlfs_mount(const fatlfs_config_t *cfg, fatlfs_t **out) {
    ...
    if (fs->cluster_size % FAT_SECTOR_SIZE) {
        free(fs);
        return FATLFS_ERR_INVAL;
    }
    fs->spc = fs->cluster_size / FAT_SECTOR_SIZE;
    fs->cluster_count = cfg->cluster_count ? cfg->cluster_count : FAT16_MAX_CLUSTERS;
    if (fs->cluster_count < FAT16_MIN_CLUSTERS) {
        fs->cluster_count = FAT16_MIN_CLUSTERS;
    }
    if (fs->cluster_count > FAT16_MAX_CLUSTERS) {
        fs->cluster_count = FAT16_MAX_CLUSTERS;
    }
    ...
    if (cfg->volume_label[0]) {
        memcpy(fs->volume_label, cfg->volume_label, 11);
    } else {
        memcpy(fs->volume_label, "FATLFS     ", 11);
    }
    ...
    fs->fc_wr.buffer = fs->cache_wr;
    fs->fc_wr.attrs = NULL;
    fs->fc_wr.attr_count = 0;
    fs->fc_rd.buffer = fs->cache_rd;
    fs->fc_rd.attrs = NULL;
    fs->fc_rd.attr_count = 0;
    fs->fc_spill.buffer = fs->cache_spill;
    fs->fc_spill.attrs = NULL;
    fs->fc_spill.attr_count = 0;
    fs->fc_spill_rd.buffer = fs->cache_spill_rd;
    fs->fc_spill_rd.attrs = NULL;
    fs->fc_spill_rd.attr_count = 0;
    for (uint32_t c = 0; c < fs->cc2; c++) { fs->owner[c].node = -1;
                                             fs->owner[c].idx = 0;
                                             fs->spillhead[c] = 0;
    }
...
int fatlfs_unmount(fatlfs_t *fs) {
    ...
    spill_rd_close(fs);
    if (fs->staging) {
        for (uint32_t c = 0; c < fs->cc2; c++) { free(fs->staging[c]);
        }
    }
    if (fs->dir_bytes) {
        for (uint32_t c = 0; c < fs->cc2; c++) { free(fs->dir_bytes[c]);
        }
    }
    free(fs->fat);
    free(fs->staging);
    ...
    free(fs->cache_rd);
    free(fs->cache_spill);
    free(fs->cache_spill_rd);
    free(fs);
    return rc;
}

uint32_t fatlfs_block_size(const fatlfs_t *fs) {
    (void)fs;
    return FAT_SECTOR_SIZE;
}
uint64_t fatlfs_block_count(const fatlfs_t *fs) {
    return fs->total_sectors;
}
...
void fatlfs_spill(fatlfs_t *fs, uint32_t max_clusters) {
    if (fs->staged_bytes > fs->staging_max) {
        spill_pressure(fs, max_clusters);
    }
}

uint32_t fatlfs_staged_bytes(const fatlfs_t *fs) {
    return fs->staged_bytes;
}
uint32_t fatlfs_staging_max(const fatlfs_t *fs) {
    return fs->staging_max;
}
...
int fatlfs_dirty(const fatlfs_t *fs) {
    return fs->staged_bytes > 0 || fs->root_dirty;
}
